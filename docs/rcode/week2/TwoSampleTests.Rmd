...
If we have very strong information against the Null, we could reject it. Now, we want to see what should be consider as "extreme" evidence against the Null. Consider the following idea:

  * Generate many "averages of 48 students" and obtain the distribution of the mean through simulation
  * Determine a cut-off point based on the distribution (the extreme end), using a pre-defined $\alpha$ value
...
The distribution (of averages) we are trying to obtain from this simulation is called the "**Null distribution**". And if our assumptions are true, then it would be around 7.3% of simulations that can be greater than 87. Hence, based on the 0.05 significance level, we will not reject this hypothesis. 
...
Example 1: Suppose we are interested in testing whether the average score is larger than 80. However, assume that the true ability of each student follows a normal distribution with SD = 15. And we actually observe the average test score as 90. Can you obtain the $p$-value of this test?

```{r class.source = NULL, eval = FALSE}
  pnorm(90 - 80, sd = 15/sqrt(48), lower.tail = FALSE)
```

Example 2: Following the same assumptions as the previous question, suppose we are interested in testing whether the average score is **smaller than 95**. Can you obtain the $p$-value of this test?
...
As we explained before, often times, We need to estimate the standard deviation $\sigma$ or the variance $\sigma^2$. This is done by calculating the sample variance
