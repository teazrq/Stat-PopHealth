...
However, this done using the `lm()` function. For example, if we want to include the longitude in the skin cancer data, we can specify two variables using the `+` sign in the right hand side of `~`:
...
Do you notice the change of significance? What do you think is the cause? Which variable(s) would you like to remove?
...
```
  ...
  3. The result still contradict with our understanding that both father and mother's heights would affect a child's height. Remove the interation term and refit the model. 
...
In fact, since `Ocean` is binary, it can be treated as either ordinal or nominal and the model would not change.

  * Can you think of examples in which not including a confuding variable would cause a misleading result? 
